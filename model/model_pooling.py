...
from model.net_vlad import NetVLAD, NetRVLAD
...
class Net(BaseModel):
    def __init__(self,
        ...
        pre_vlad_feat_sizes = {"lms": 128,
                               "vggish": 128, 
                               "vggsound": 512, 
                               "panns_cnn10": 512,
                               "panns_cnn14": 2048, 
                               "efficient_latent":1408}
        pre_vlad_feat_sizes = {key: val for key, val in pre_vlad_feat_sizes.items()
                               if feat_aggregation[key]["temporal"] in ["vlad","rvlad"]}
        ...
        for mod in audio_dims.keys():
            if feat_aggregation[mod]["temporal"] in ["vlad", "rvlad"]:
                ...
                self.audio_pooling[mod] = nn.LSTM(input_size=audio_dims[mod][1],
        ...
        self.Pool = PoolingModule(audio_dims = audio_dims,
                                  text_dim = text_dim,
    ...
    def forward(self, experts, text, expert_masks, text_token_masks, ind):
        ...
        for mod in self.modalities:
            if self.feat_aggregation[mod]['temporal'] == "meanP":
            ...
            elif self.feat_aggregation[mod]['temporal'] in ["vlad","rvlad"]:
                ...
                if self.training: self.audio_pooling[mod].flatten_parameters()
                ...
                position_ids = torch.arange(seq_length, dtype=torch.long, device = audio_original.device)
        ...
        elif self.text_agg in ["vlad", "rvlad"]:
        ...
        if len(aggregated_text.size()) > 2:
            aggregated_text = aggregated_text / (aggregated_text.norm(p=2, dim=-1, keepdim = True) + 1e-8) 
            aggregated_text = self._mean_pooling_for_similarity(aggregated_text, text_token_masks)
            aggregated_text = aggregated_text / (aggregated_text.norm(p=2,dim = -1, keepdim =True) + 1e-8)

        aggregated_text = aggregated_text.view(B, captions_per_audio, -1)
        return self.Pool(aggregated_text, aggregated_experts,ind) 
